...
enum { HORIZ_SHORT_SHORT = 0, HORIZ_SHORT_LONG, HORIZ_LONG_SHORT, HORIZ_LONG_LONG };
...
#else
#define TIFFMOTOLONG(p) \
  (((uint32_t)(*p) << 24UL) + ((uint32_t)(*(p + 1)) << 16UL) + ((uint32_t)(*(p + 2)) << 8UL) + (uint32_t)(*(p + 3)))
#endif  // __AVR__
...
typedef struct {
  uint16_t
      bitmapOffset;   // Offset to compressed bitmap data for this glyph (starting from the end of the BB_GLYPH[] array)
...
typedef struct bbg_small {
  ...
  int8_t struct_padding;  // pad the structure to 8 bytes to prevent alignment problems on different target
                          // compilers/settings
} BB_GLYPH_SMALL;
